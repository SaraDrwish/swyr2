          ...
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                filter === category
                  ? "bg-gradient-to-r from-blue-900 to-orange-500 text-white"
          ...
          <AnimatePresence>
            {filteredProjects.map((project) => (
              <motion.div
                ...
                className="bg-background rounded-3xl shadow-lg overflow-hidden hover-lift transition-all duration-300 ease-in-out border-2 border-transparent hover:border-orange-500/20"
              >
                ...
                <div className="p-6">
                  <div className="text-sm font-medium text-orange-600 mb-1">{project.category}</div>
                  <h3 className="text-xl font-semibold text-foreground mb-2">{project.title}</h3>
                  <a
                    href="https://www.flowersandsaints.com.au"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-900 hover:text-orange-500 hover:underline inline-flex items-center transition-colors"
                  >
                    View Project
                    <svg
